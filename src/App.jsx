...
function A4CustomersPrint({ rows, active, profile, periodLabel }) {
  ...
  return <section className="print-only a4-page customer-report">
    <div className="print-meta"><strong>Total Customers: {rows.length}</strong><span>Period: {periodLabel}</span><span>Kul Udhar: {money(totalDebit)}</span><span>Kul Wasooli: {money(totalCredit)}</span><span>Kul Total: {money(totalAll)}</span></div>
    <table><thead><tr className="shop-name-row"><th colSpan="8">{profile.shop_name || 'Mandi Khata'}</th></tr><tr className="date-row"><th colSpan="8">Report Date: {new Date().toLocaleDateString('en-IN')}</th></tr><tr><th>#</th><th>Customer Name</th><th>Phone</th><th className="text-right">Udhar</th><th className="text-right">Wasooli</th><th className="text-right">Rehta Baqi</th><th className="text-right">Kul Total</th><th className="text-right">Today Wasooli</th></tr></thead><tbody>{rows.map((row, index) => <tr key={row.id}><td>{index + 1}</td><td>{row.name}</td><td>{row.phone || '—'}</td><td className="text-right">{money(row.debit)}</td><td className="text-right">{money(row.credit)}</td><td className="text-right">{money(Math.abs(Number(row.net_balance || 0)))}<small>{balanceLabel(row.net_balance)}</small></td><td className="text-right">{money(Number(row.debit || 0) + Number(row.credit || 0))}</td><td></td></tr>)}</tbody><tfoot><tr><th colSpan="7" className="text-right">Totals</th><th></th></tr><tr><th colSpan="3" className="text-right">Kul</th><th className="text-right">{money(totalDebit)}</th><th className="text-right">{money(totalCredit)}</th><th className="text-right">{money(Math.abs(totalBalance))}</th><th className="text-right">{money(totalAll)}</th><th></th></tr></tfoot></table>
  </section>;
}
